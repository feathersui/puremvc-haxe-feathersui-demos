...
package org.puremvc.haxe.demos.openfl.helloopenfl.view.components;
...
class HelloSprite extends Sprite {
	// Event name to tell Mediator to create a new sprite
	public static final SPRITE_DIVIDE = "spriteDivide";

	// id, size and color
	public var id:String;
	public var size:Int = 75;
	public var color:Int = 0x0000FF;

	// boundaries
	public var xBound:Int = 400;
	public var yBound:Int = 400;

	// xy Location
	public var xLoc:Int = 175;
	public var yLoc:Int = 30;
	...
	**/
	public function new(id:String, params:Array<Int> = null) {
		super();
		this.id = id;
		if (params != null) {
			xLoc = params[0];
	...
	private function handleMouseMove(event:MouseEvent):Void {
		if (child.x < xLoc) {
			xVec = -Std.int(Math.abs(xVec));
		} else if (child.x > xLoc) {
			xVec = Std.int(Math.abs(xVec));
		}

		if (child.y < yLoc) {
			yVec = -Std.int(Math.abs(yVec));
		} else if (child.y > yLoc) {
			yVec = Std.int(Math.abs(yVec));
		}

		xLoc = Std.int(child.x);
		yLoc = Std.int(child.y);
	...
	// scale the sprite
	public function scaleSprite(delta:Int):Void {
		size += (delta);
		if (size <= 5)
			size = 5;
		if (size >= 150)
	...
	public var newSpriteState(get, never):Array<Int>;

	private function get_newSpriteState():Array<Int> {
